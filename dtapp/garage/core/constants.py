...
# Okta issuer — the Drivetrain SPA Okta; single const across envs (FE + BE both validate against it).
OKTA_ISSUER = "https://auth.drivetrain.ai/oauth2/default"
...
KICKSTART_DELETE_FLAG = "kickstart-delete"  # LD: gates deleting a real kickoff (FE + BE); Paaras-only
...
ASK_SRE_SLACK_CHANNEL_ID = "C02LF13SRAA"   # #ask-sre
SRE_SUBTEAM_ID = "S0B52MXCT0C"             # @sre-team usergroup (<!subteam^…> mention)
SENDGRID_FROM_EMAIL = "drivetrainteam@drivetrain.ai"
SENDGRID_FROM_NAME = "Drivetrain Team"
...
# Academy-access-request body (MARKT-2843): column defaults + the cc-praveen mention markup (pings on
# create). The table is assembled from _ACADEMY_COLUMNS in completion._academy_desc.
LINEAR_ACADEMY_MANAGER = "Jason"   # "Academy Manager (Onboarding/Tenant PoC)" column default
LINEAR_ACADEMY_COURSE = "V3"       # "Course to assign" column default
LINEAR_ACADEMY_CC = 'cc <user id="676b25a8-dad8-4918-ba6f-5d72f9d4345b">praveen</user>'

# ── Monday template board (created inside each tenant's workspace) ──
MONDAY_BOARD_TEMPLATE_ID = "13747856"          # template_center template — same across all envs
...
MONDAY_ACCOUNT_SLUG = "drivetrainai"            # <slug>.monday.com — for the workspace deep-link
MONDAY_WORKSPACE_KIND = "open"                   # Monday WorkspaceKind: "open" (account-wide) | "closed" (subscribers only)

# ── Fixed Drivetrain references (same across all envs — not env-resolved) ──
SLACK_WORKSPACE_URL = "https://drivetrainai.slack.com"  # workspace base for channel deep-links
TENANT_SITE_DOMAIN = "drivetrain.ai"            # customer Drive tenant site: <subdomain>.<domain>
CONSULTANT_SHEET_ID = "1S1pd3yjOMwtNvlYKVX4p_ybOY9W6LVkxjLc-RO1ILCY"  # consultant roster Google Sheet
...
CONSULTANT_REMINDER_CRON = "5 14 * * *"    # daily 2:05 PM IST — nudge Paaras to assign a consultant
